...
public class feriaDao {
    ...
    public boolean updateStatus(String id_feria) {
        ...
        String montoquery = "SELECT monto FROM feria WHERE id_feria = ?";
        ...
        try {
            ...
            // Obtener el monto total de la feria
            pst = conn.prepareStatement(montoquery);
            pst.setString(1, id_feria);
            rs = pst.executeQuery();
            double totalMonto = 0;
            while (rs.next()) {
            ...
            if (totalIngresos != 0) {
                rentabilidad = ((totalIngresos - totalEgresos) / totalIngresos) * 100;
            }

            // Establecer el estado como "viable" o "inviable" según el porcentaje de rentabilidad y totalMonto
            String estado;
            if (rentabilidad > 30 && totalMonto >= totalEgresos) {
                estado = "VIABLE";
            } else {
                estado = "INVIABLE";
            }
    ...
    public boolean updateStatusApro(String id_feria, String comentarios, String statusApro) {
        String query = "UPDATE feria SET status_aprov = ?, comentarios = ? WHERE id_feria = ?";
        try {
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
            pst.setString(1, statusApro);
            pst.setString(2, comentarios);
            pst.setString(3, id_feria);
            pst.execute();
            return true;
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error al modificar el estado de aprobación de la feria " + e);
            return false;
        }
    }

    public List<feria> listFeriaCalculoQuery(String id_feria) {
        List<feria> listFerias = new ArrayList<>();
        String query = "WITH TotalIngresos AS ("
                + "    SELECT id_feria, SUM(precio * cantidad) AS total_ingresos "
                + "    FROM ingreso "
                + "    WHERE id_feria = ? "
                + "    GROUP BY id_feria "
                + "), "
                + "TotalEgresos AS ("
                + "    SELECT id_feria, SUM(precio * cantidad) AS total_egresos "
                + "    FROM egreso "
                + "    WHERE id_feria = ? "
                + "    GROUP BY id_feria "
                + ") "
                + "SELECT f.id_feria, f.monto, f.comentarios, ti.total_ingresos, te.total_egresos, "
                + "       (ti.total_ingresos - te.total_egresos) AS utilidad, "
                + "       ((ti.total_ingresos - te.total_egresos) / ti.total_ingresos * 100) AS rentabilidad, "
                + "       f.status_aprov "
                + "FROM feria f "
                + "INNER JOIN TotalIngresos ti ON f.id_feria = ti.id_feria "
                + "INNER JOIN TotalEgresos te ON f.id_feria = te.id_feria "
                + "WHERE f.id_feria = ? "
                + "ORDER BY f.id_feria"; // Puedes cambiar el campo para ordenar según tus necesidades

        try ( Connection conn = cn.getConnection();  PreparedStatement pst = conn.prepareStatement(query)) {
            pst.setString(1, id_feria); // Establecer el valor del parámetro id_feria en la consulta
            pst.setString(2, id_feria); // Establecer el valor del parámetro id_feria para TotalEgresos
            pst.setString(3, id_feria); // Establecer el valor del parámetro id_feria para la cláusula WHERE
            try ( ResultSet rs = pst.executeQuery()) {
                while (rs.next()) {
                    feria fair = new feria();
                    fair.setId(rs.getString("id_feria"));
                    fair.setMonto(rs.getDouble("monto"));
                    fair.setIngresos(rs.getDouble("total_ingresos"));
                    fair.setEgresos(rs.getDouble("total_egresos"));
                    fair.setUtilidad(rs.getDouble("utilidad"));
                    fair.setRentabilidad(rs.getDouble("rentabilidad"));
                    fair.setStatusApro(rs.getString("status_aprov"));
                    fair.setComentarios(rs.getString("comentarios"));
                    listFerias.add(fair);
                }
            }
        } catch (SQLException e) {
